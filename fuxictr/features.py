...
class FeatureMapAbsTime(FeatureMap):
    def __init__(self, dataset_id, data_dir):
        super(FeatureMapAbsTime, self).__init__(dataset_id=dataset_id, data_dir=data_dir)

    def sum_emb_out_dim_wo_time(self, time_keys, feature_source=[]):
        if type(feature_source) != list:
            feature_source = [feature_source]
        total_dim = 0
        for feature, feature_spec in self.features.items():
            if feature_spec["type"] == "meta" or feature in time_keys:
                continue
            if len(feature_source) == 0 or feature_spec.get("source") in feature_source:
