...
class Normalizer(object):
    ...
    def encode_sequence(self, texts):
        ...
        for text in texts:
            if pd.isnull(text) or text == '':
                sequence_list+=[]
                sequence_len_list.append(0)
            else:
                seq = list(map(float, text.split(self.splitter)))
                seq_len = len(seq)
                sequence_list+=seq
                sequence_len_list.append(seq_len)
        sequence_list = self.normalize(np.array(sequence_list))
        sequence_2d_list = []

        right_len = 0
        left_len = 0
        for sequence_len in sequence_len_list:
            right_len += sequence_len
            sequence_2d_list.append(sequence_list[left_len: right_len])
            left_len += sequence_len
        sequence_2d_list = pad_sequences(sequence_2d_list, maxlen=self.max_len, value=self.padding_idx,
                                      padding=self.padding, truncating=self.padding, dtype='float')
        return np.array(sequence_2d_list)
