...
class FeatureProcessor(object):
    ...
    def preprocess(self, ddf):
        ...
        for col in all_cols:
            ...
            if name in ddf.columns:
                ...
                if not(col.get("type", None) == "sequence" and self.dtype_dict[name] == float):
    ...
    def fit(self, train_ddf, min_categr_count=1, num_buckets=10, **kwargs):    
        ...
        for name, spec in self.feature_map.features.items():
            if spec["type"] == "numeric":
                ...
                if (spec["dtype"] == "str" and spec["type"] == "sequence") or spec["type"] == "categorical":
    ...
    def fit_meta_col(self, col, col_values):
        ...
        feature_dtype = col["dtype"]
        self.feature_map.features[name] = {"type": feature_type, "dtype": feature_dtype}
    ...
    def fit_numeric_col(self, col, col_values):
        ...
        self.feature_map.features[name] = {"source": feature_source,
                                                "type": feature_type, "dtype": feature_dtype}
    ...
    def fit_categorical_col(self, col, col_values, min_categr_count=1, num_buckets=10):
        ...
        self.feature_map.features[name] = {"source": feature_source,
                                                "type": feature_type, "dtype": feature_dtype}
        padding_idx = col.get("padding_idx", 0)
        if "feature_encoder" in col:
            self.feature_map.features[name]["feature_encoder"] = col["feature_encoder"]
        ...
        if "category_processor" not in col:
            ...
            self.feature_map.features[name].update({"padding_idx": padding_idx,
    ...
    def fit_sequence_col(self, col, col_values, min_categr_count=1):
        ...
        padding = col.get("padding", "pre") # "post" or "pre"
        if feature_dtype == "str":
            padding_idx = col.get("padding_idx", 0)
            ...
            assert "share_embedding" in col, "please ensure that the sequence field and the target field share the same embedding"
            padding_idx = col.get("padding_idx", -1)
            self.feature_map.features[name]["share_embedding"] = col["share_embedding"]
            self.feature_map.features[name]["padding_idx"] = padding_idx
            self.feature_map.features[name]["max_len"] = max_len
            self.feature_map.features[name]["dtype"] = feature_dtype


            self.processor_dict[name + "::normalizer"] = self.processor_dict[col["share_embedding"] + "::normalizer"]
            self.processor_dict[name + "::normalizer"].max_len = max_len
            self.processor_dict[name + "::normalizer"].splitter = splitter
            self.processor_dict[name + "::normalizer"].padding_idx = padding_idx
            self.processor_dict[name + "::normalizer"].padding = padding        
        ...
        for feature, feature_spec in self.feature_map.features.items():
            if feature in ddf.columns:
                ...
                if feature_type == "meta":
                    ...
                    if normalizer:
                         col_values = normalizer.normalize(col_values)
                    ...
                    if feature_dtype == "str":
                        ...
                        data_dict[feature] = self.processor_dict.get(feature + "::normalizer").encode_sequence(col_values)
    ...
    def save_vocab(self, vocab_file):
        ...
        for feature, spec in self.feature_map.features.items():
            if (spec["type"] == "sequence" and spec["dtype"] == "str") or spec["type"] == "categorical":
