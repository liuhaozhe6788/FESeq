        ...
        for feature, feature_spec in self._feature_map.features.items():
            ...
            if self.is_required(feature):
                ...
                if feature_spec["type"] == "numeric":
                    ...
                    embedding_matrix = nn.Embedding(num_embeddings if num_embeddings else feature_spec["vocab_size"], 
                                                    feat_emb_dim, 
                    ...
                    if feature_spec.get("dtype", None) == "float":
                        self.embedding_layers[feature] = nn.Linear(1, feat_emb_dim, bias=False)
                    elif feature_spec.get("dtype", None) == "str":
                        padding_idx = feature_spec.get("padding_idx", None)
                        embedding_matrix = nn.Embedding(num_embeddings if num_embeddings else feature_spec["vocab_size"], 
                                                        feat_emb_dim, 
                                                        padding_idx=padding_idx)
