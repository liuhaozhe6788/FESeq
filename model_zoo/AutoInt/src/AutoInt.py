...
class AutoInt(BaseModel):
    def __init__(self, 
        ...
        self.self_attention = nn.Sequential(
            *[MultiHeadSelfAttention(embedding_dim if i == 0 else attention_dim,
                                     ...
                                     save_attn_matrix=self._save_attn_matrix,
                                     last_layer=(i==attention_layers-1)) \
    ...
    def forward(self, inputs):
        ...
        if self._save_attn_matrix:
            attention_out, attention_score = self.self_attention(feature_emb)
        else:
        ...
        if self._save_attn_matrix:
            return_dict = {"y_pred": y_pred, "attn": attention_score}
        else:
...
class MultiHeadSelfAttention(nn.Module):
    ...
    def __init__(self, input_dim, attention_dim=None, num_heads=1, dropout_rate=0., 
                 use_residual=True, use_scale=True, layer_norm=True, save_attn_matrix=False, last_layer=False):
        super(MultiHeadSelfAttention, self).__init__()
        if attention_dim is None:
            attention_dim = input_dim
        assert attention_dim % num_heads == 0, \
               "attention_dim={} is not divisible by num_heads={}".format(attention_dim, num_heads)
        self.save_attn_matrix = save_attn_matrix
    ...
    def forward(self, X):
        ...
        output = output.relu()
        if self.save_attn_matrix and self.last_layer:
