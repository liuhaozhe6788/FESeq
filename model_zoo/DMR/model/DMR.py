...
class DMR(BaseModel):
    ...
    def forward(self, inputs):
        ...
        for i, field in enumerate(self.target_field):
            ...
            if self.feature_map.features[self.sequence_field[i]]["dtype"] == "str":
                pad_mask = X[self.sequence_field[i]].long() > 0
            elif self.feature_map.features[self.sequence_field[i]]["dtype"] == "float":
                pad_mask = X[self.sequence_field[i]].float() != self.feature_map.features[self.sequence_field[i]]["padding_idx"]
            # item2item network
            ...
            if self.enable_u2i_rel:
                # user2item network
                neg_field = "neg_"+field
                if neg_field in self.neg_seq_field:
                    neg_emb = self.get_out_embedding(neg_field, field, X) \
...
class User2ItemNet(nn.Module):
    ...
    def forward(self, target_emb, sequence_emb, context_emb, sequence_emb2, neg_emb=None, mask=None):
        ...
        if neg_emb is not None:
            ...
            neg_prob = torch.sum(user_embs[:, -2, :] * neg_emb[:, -1, :], dim=-1).sigmoid()
